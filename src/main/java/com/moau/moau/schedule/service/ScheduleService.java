...
public class ScheduleService {
    ...
    // [✅ 실제 로직 구현]
    public List<ScheduleResponse> getGroupSchedules(Long groupId, int year, int month) {
        ...
        List<Schedule> schedules = scheduleRepository.findByGroup_IdAndStartsAtBetween(groupId, startOfMonth, endOfMonth);

        // 3. 가져온 엔티티 목록을 DTO 목록으로 변환합니다. (Java Stream API 사용)
        return schedules.stream()
                .map(ScheduleResponse::from) // .map(schedule -> ScheduleResponse.from(schedule)) 와 동일
